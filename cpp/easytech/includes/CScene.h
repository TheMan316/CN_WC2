...
#include "ecLibrary.h"
extern ecImage* MapBackgroundImage ;
struct SaveArmyInfo;
struct CArea;
//添加新属性，别忘了去init函数写初始化代码。
struct CArmy {
    enum Card {
        LightArtillery = 0,//轻炮
        EngineeringCorps  , //工程
    ...
    short Supplies; //军队后勤
    short MaxSupplies;
    bool HasReceivedSupplies; //是否已经获得了补给
#ifdef __cplusplus
    void Init(ArmyDef *, CCountry *);

    bool Is_collapsed();
    bool Is_eliteArmy();
    void Set_suppliesStatus();
...
public:
    ...
    void Dijkstra(int pathType, int startAreaID,int maxRIngCount);
...
private:
    ...
    void Set_lastID_inShortestPath(int targetAreaID, int pathType, CArea* armyArea);
};
...
struct CArea {
    ...
    int TargetType; //1是“可选择箭头”， 2是远程攻击箭头，3是移动箭头，4是近战攻击箭头
...
bool Is_existTheServicesType(std::vector< CountryArmyDef::servicesType>&  list_armyType);
    ...
    int Is_hasExport();
    int Get_oil();
    void Set_transportSupplying_toArmiesAround(int maxDistance);
...
//这是移除军队（用于单位移动后重置这个地块的信息），军队死亡请配合delete
    ...
    void RenderArmy(float x, float w, int, CArmy *);
...
#ifdef __cplusplus
    int CalRealPosX(int x, int w); //新函数
    void Apply();
...
struct CBackground {
    ...
    void Init(int MapID, int TotalX, int TotalY, int TotalW, int TotalH, float EnableX, float EnableY,
