...
#include "CActionAI.h"
 ecImage* MapBackgroundImage ;
float g_contenscalefactor = 2.0;
extern bool g_NewLoadMap;
extern int g_MapW;
...
//    ExtendedBackgroundImage = new ecImage *[this->MapCount];
    char FileName[32];
    auto* MapTexture = ecGraphics::Instance()->LoadETCTexture(g_MapName);
    if (MapBackgroundImage == nullptr)
        MapBackgroundImage = new ecImage();
    MapBackgroundImage->Init(MapTexture, 0, 0, g_MapW, g_MapH);
...
//    for (i = 0; i < this->MapCount; i++) {
//        //此处做过优化
//        if (ExtendedBackgroundImage[i] != nullptr && camera->IsRectInCamera(CurrentPos[0],CurrentPos[1],500,500))
...
//    }
    ...
    GUIRect Rect = {{camera->CenterPos[0] - camera->ScreenHalfSize[0] / camera->Scale, camera->CenterPos[1] - camera->ScreenHalfSize[1] / camera->Scale },
                    { camera->ScreenHalfSize[0] * 2 / camera->Scale, camera->ScreenHalfSize[1] * 2 / camera->Scale }};
    MapBackgroundImage->SetTextureRect(Rect.Pos[0], Rect.Pos[1], Rect.Size[0], Rect.Size[1]);
    MapBackgroundImage->Render( Rect.Pos[0], Rect.Pos[1]);
