...
bool CCamera::SetPos(float x, float y, bool border) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (fCenterPos < fMinPos && (i || !g_OpenMapLoop)) {
            fCenterPos = fMinPos;
            bFixed = true;
        }
        if (fCenterPos > fMaxPos &&(i ||!g_OpenMapLoop)) {
            fCenterPos = fMaxPos;
            bFixed = true;
        }
        if (fMinPos > fMaxPos && (i ||!g_OpenMapLoop))
            fCenterPos = (fMinPos + fMaxPos) * 0.5;
        if (bFixed)
            this->CenterPos[i] = fCenterPos;
    }
    if (g_OpenMapLoop){
        int CameraX_Left = CenterPos[0] - ScreenHalfSize[0] / Scale;
        int CameraX_Right = CameraX_Left + ScreenHalfSize[0] * 2 / Scale;
        if (CameraX_Left <= -MapSize[0] ) {
            bFixed = true;
            this->CenterPos[0] += MapSize[0];
        }
        else if (CameraX_Right >= MapSize[0]) {
            bFixed = true;
            this->CenterPos[0] -= MapSize[0];
        }
    }
...
int CCamera::CalRealPosX(int x, int w) {
    if (g_OpenMapLoop) {
        int CameraX_Left = CenterPos[0] - ScreenHalfSize[0] / Scale;
        int CameraX_Right = CameraX_Left + ScreenHalfSize[0] * 2 / Scale;
        if (x + w < CameraX_Left)
            x += MapSize[0];
        else if (x >= CameraX_Right)
            x -= MapSize[0];
    }
    return x;
}
...
void CBackground::RenderBox(CCamera *camera, float x, float y, float w, float h) {
    if (g_OpenMapLoop) {
        x = camera->CenterPos[0] - camera->ScreenHalfSize[0] / camera->Scale;
        for (int i = 0; i < (int) ((w + 139.0 - 1.0) / 139.0); i++) {
