...
void JNI_Fun::Show_testForm(const char* title,int num) {
    JNIEnv* env = nullptr;
    if (g_vm != nullptr && (g_vm->GetEnv((void**) &env, JNI_VERSION_1_6) == JNI_OK)) {
        jclass activity = env->FindClass("com/easytech/wc2/BaseEvent");
        jmethodID ff = env->GetStaticMethodID(activity, "展示_文本测试_窗口", "(Ljava/lang/String;I)V");
        env->CallStaticVoidMethod(activity, ff, env->NewStringUTF(title),num);
    }
}
